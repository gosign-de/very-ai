...
const logger = createLogger({ feature: "api/chat/deepseek/pdf" });
...
export async function POST(request: Request) {
  try {
    ...
    pdfDoc.setSubject(
      `pdfContent:${pdfContent} tableData:${JSON.stringify(tableData)}`,
    );
    ...
    const contentParts = pdfContent.includes("{{TABLE}}")
      ? pdfContent.split("{{TABLE}}")
      : [pdfContent];
    ...
    return new Response(
      JSON.stringify({
        message: `pdfFileId:${fileData.id}<<END>><<pdfContentStart>>${pdfContent} tableData:${JSON.stringify(tableData)}<<pdfContentEnd>>`,
      }),
      {
        status: 200,
      },
