drop function if exists "public"."get_user_stats"(time_period text, model_param text, role_param text, page_size integer, page_number integer);
...
BEGIN
    ...
    ELSIF time_period = 'this_month' THEN
        ...
        time_condition := 'm.created_at >= DATE_TRUNC(''month'', CURRENT_DATE) AND m.created_at < CURRENT_DATE + INTERVAL ''1 day''';
    ...
    ELSIF time_period = 'this_year' THEN
        -- From Jan 1 to current date
        time_condition := 'm.created_at >= DATE_TRUNC(''year'', CURRENT_DATE) AND m.created_at < CURRENT_DATE + INTERVAL ''1 day''';
