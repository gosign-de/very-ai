drop function if exists "public"."get_user_analytics"(start_date date, end_date date);

drop function if exists "public"."get_users_analytics"(start_date date, end_date date);

set check_function_bodies = off;

CREATE OR REPLACE FUNCTION public.fetch_user_analytics(start_date date DEFAULT NULL::date, end_date date DEFAULT NULL::date)
 RETURNS TABLE(id uuid, username text, "aiModelRequests" jsonb, "aiAssistantUses" bigint, "lastSignIn" timestamp with time zone)
 LANGUAGE plpgsql
 SECURITY DEFINER
AS $function$DECLARE
    total_users INT;
