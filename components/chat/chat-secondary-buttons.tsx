"use client";

import { useChatHandler } from "@/components/chat/chat-hooks/use-chat-handler";
import { ChatbotUIContext } from "@/context/context";
import {
  IconInfoCircle,
  IconMessagePlus,
  IconArchive,
} from "@tabler/icons-react";
import { FC, useContext } from "react";
import { WithTooltip } from "../ui/with-tooltip";
import { useTranslation } from "react-i18next";
import { useArchiveChatHandler } from "@/components/chat/chat-hooks/use-archive-chat";
import { ForkChat } from "./chat-fork";
interface ChatSecondaryButtonsProps {}

export const ChatSecondaryButtons: FC<ChatSecondaryButtonsProps> = ({}) => {
  const { t } = useTranslation();
  const { selectedChat } = useContext(ChatbotUIContext);

  const { handleNewChat } = useChatHandler();
  const { archiveChat } = useArchiveChatHandler();
  return (
    <>
      {selectedChat && (
        <>
          <WithTooltip
            delayDuration={200}
            display={<div>{t("Fork this Chat")}</div>}
            trigger={
              <div className="mt-1">
                <ForkChat chat={selectedChat} />
              </div>
            }
          />
          <WithTooltip
            delayDuration={200}
            display={<div>{t("Archive Chat")}</div>}
            trigger={
              <div className="mt-1">
                <IconArchive
                  className="cursor-pointer hover:opacity-50"
                  size={24}
                  onClick={() => archiveChat(selectedChat)}
                />
              </div>
            }
          />
          <WithTooltip
            delayDuration={200}
            display={
              <div>
                <div className="text-xl font-bold">{t("Chat Info")}</div>

                <div className="mx-auto mt-2 max-w-xs space-y-2 sm:max-w-sm md:max-w-md lg:max-w-lg">
                  <div>
                    {t("Model")}: {selectedChat.model}
                  </div>
                  <div>
                    {t("Image Model")}: {selectedChat.image_model}
                  </div>

                  <div>
                    {t("Prompt")}: {selectedChat.prompt}
                  </div>

                  <div>
                    {t("Temperature")}: {selectedChat.temperature}
                  </div>
                  <div>
                    {t("Context Length")}: {selectedChat.context_length}
                  </div>

                  <div>
                    {t("Profile Context")}:{" "}
                    {selectedChat.include_profile_context
                      ? t("Enabled")
                      : t("Disabled")}
                  </div>
                  <div>
                    {t("Workspace Instructions")}:{" "}
                    {selectedChat.include_workspace_instructions
                      ? t("Enabled")
                      : t("Disabled")}
                  </div>

                  <div>{t("Embeddings Provider")}: Azure AI</div>
                </div>
              </div>
            }
            trigger={
              <div className="mt-1">
                <IconInfoCircle
                  className="cursor-default hover:opacity-50"
                  size={24}
                />
              </div>
            }
          />

          <WithTooltip
            delayDuration={200}
            display={<div>{t("Start a new chat")}</div>}
            trigger={
              <div className="mt-1">
                <IconMessagePlus
                  className="cursor-pointer hover:opacity-50"
                  size={24}
                  onClick={_event => handleNewChat()}
                />
              </div>
            }
          />
        </>
      )}
    </>
  );
};
