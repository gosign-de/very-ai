...
export const CollectionFileSelect: FC<CollectionFileSelectProps> = ({
  ...
  const syncedSelectedFiles = selectedCollectionFiles.map(selectedFile => {
    const matchingFile = files.find(file => file.id === selectedFile.id);
    if (matchingFile) {
      return {
        ...selectedFile,
        ...
        >
          <div className="flex items-center">
            <div className="ml-2 flex items-center">
              {selectedCollectionFiles.length} {t("files selected")}
            </div>
          </div>
        ...
        {syncedSelectedFiles
          ...
          .map(file => (
            <CollectionFileItem
              key={file.id}
              file={file}
              selected={syncedSelectedFiles.some(
