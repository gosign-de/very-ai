...
import { CollectionFile, ContentType, DataItemType, LLMID } from "@/types";
import { FC, useContext, useEffect, useRef, useState } from "react";
import profile from "react-syntax-highlighter/dist/esm/languages/hljs/profile";
import { toast } from "sonner";
import { SidebarDeleteItem, CustomSidebarDelete } from "./sidebar-delete-item";
...
export const SidebarUpdateItem: FC<SidebarUpdateItemProps> = ({
  ...
  const fetchDataFunctions = {
    ...
    "group-assistants": async (assistantId: string) => {
      const assistantFiles = await getAssistantFilesByAssistantId(assistantId);
      setStartingAssistantFiles(assistantFiles.files);
      ...
      const assistantUser = await getGroupAssistantByUserId(assistantId);
      ...
      // Admins can delete group assistants from all users
      if (isOwner || isAdmin) {
        setHasAccess(true);
      } else {
        setHasAccess(false);
  ...
  // Define workspace management functions by content type
  const workspaceManagementFunctions = {
    chats: {
      deleteFn: null, // Chats don't have workspace associations
      createFn: null,
      itemIdKey: "chat_id",
    },
    presets: {
      deleteFn: deletePresetWorkspace,
      createFn: createPresetWorkspaces as any,
      itemIdKey: "preset_id",
    },
    prompts: {
      deleteFn: deletePromptWorkspace,
      createFn: createPromptWorkspaces as any,
      itemIdKey: "prompt_id",
    },
    files: {
      deleteFn: deleteFileWorkspace,
      createFn: createFileWorkspaces as any,
      itemIdKey: "file_id",
    },
    collections: {
      deleteFn: deleteCollectionWorkspace,
      createFn: createCollectionWorkspaces as any,
      itemIdKey: "collection_id",
    },
    assistants: {
      deleteFn: deleteAssistantWorkspace,
      createFn: createAssistantWorkspaces as any,
      itemIdKey: "assistant_id",
    },
    "group-assistants": {
      deleteFn: deleteAssistantWorkspace,
      createFn: createAssistantWorkspaces as any,
    ...
    tools: {
      deleteFn: deleteToolWorkspace,
      createFn: createToolWorkspaces as any,
      itemIdKey: "tool_id",
    },
    models: {
      deleteFn: deleteModelWorkspace,
      createFn: createModelWorkspaces as any,
      itemIdKey: "model_id",
    },
  };
  ...
  const handleUpdate = async () => {
    try {
      ...
      const workspaceFunctions = workspaceManagementFunctions[contentType];

      if (!updateFunction) return;

      let updatedItem = await updateFunction(item.id, updateState);

      // Handle workspace updates if the content type supports it
      if (
        workspaceFunctions &&
        workspaceFunctions.deleteFn &&
        workspaceFunctions.createFn
      ) {
        await handleWorkspaceUpdates(
          startingWorkspaces,
          selectedWorkspaces,
      ...
      // Update selectedAssistant if the updated item is the currently selected assistant
      if (
        (contentType === "assistants" || contentType === "group-assistants") &&
        selectedAssistant &&
        selectedAssistant.id === item.id
      ) {
        // Type assertion since we know this is an assistant based on contentType check
        const updatedAssistant = updatedItem as Tables<"assistants">;
      ...
      toast.error(
        t("Error updating {{contentType}}. {{error}}", {
          contentType: contentType.slice(0, -1),
          error,
        }),
      );
      ...
      >
        <div className="grow overflow-auto">
          ...
          <div className="mt-4 space-y-3">
            {workspaces.length > 1 && contentType !== "group-assistants" && (
        ...
        <SheetFooter className="mt-2 flex flex-row justify-between">
          ...
          <div className="flex grow justify-end space-x-2">
            ...
            {contentType === "group-assistants" && hasAccess === false ? (
              <WithTooltip
                display={t(
                  "You don't have access to update this Group Assistant",
                )}
                trigger={
                  <Button
                    ref={buttonRef}
...
interface CustomSidebarUpdateItemProps {
  isTyping: boolean;
  item: DataItemType;
  contentType: ContentType;
  children: React.ReactNode;
...
export const CustomSidebarUpdate: FC<CustomSidebarUpdateItemProps> = ({
  item,
  contentType,
  children,
  renderInputs,
  ...
  const handleUpdate = async () => {
    try {
      ...
      if (contentType === "group-assistants") {
        toast.success(t("Group-Assistant updated successfully"));
      } else {
        toast.success(
          t(`{{contentType}} updated successfully`, {
            contentType: contentType.slice(0, -1),
      ...
      if (contentType === "group-assistants") {
        toast.error(
          t("Error updating {{groupAssistant}}. {{error}}", {
  ...
  return (
    <Sheet open={isOpen} onOpenChange={setIsOpen} modal>
      ...
      <SheetContent
        className="flex min-w-[300px] flex-col justify-between sm:min-w-[400px]"
        side="left"
        onKeyDown={handleKeyDown}
      >
        <div className="grow overflow-auto">
        ...
        <SheetFooter className="mt-2 flex justify-between">
          <CustomSidebarDelete item={item} contentType={contentType} />
