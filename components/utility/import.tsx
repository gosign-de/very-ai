...
const logger = createClientLogger({ component: "Import" });
    ...
    <>
      ...
      {isOpen && (
        <Dialog open={isOpen} onOpenChange={setIsOpen}>
          ...
          >
            <DialogHeader>
              <div className="text-2xl font-bold">{t("Import Data")}</div>

              <DialogDescription>
                {t("Import data from a JSON file(s).")}
              </DialogDescription>
            </DialogHeader>
            ...
            <div className="max-w-[560px] space-y-4">
              ...
              {Object.entries(importCounts).map(([key, value]) => {
                if (value > 0) {
                  return <div key={key}>{`${t(key)}: ${value}`}</div>;
                }
                return null;
              })}
